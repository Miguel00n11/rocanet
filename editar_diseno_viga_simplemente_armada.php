...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql_update = "UPDATE diseno_viga_simplemente_armada 
                   SET codigo_diseno='$codigo_diseno', 
                       obra='$obra', 
                       ancho_viga='$ancho_viga', 
                       altura_viga='$altura_viga', 
                       fc='$fc', 
                       fy='$fy', 
                       recubrimiento_inferior='$recubrimiento_inferior', 
                       mu='$mu' 
                   WHERE id_viga=$id_viga";

    if ($conexion->query($sql_update) === TRUE) {
        echo '<div class="alert alert-success" role="alert">Diseño actualizado exitosamente. Redirigiendo...</div>';
        echo '<script>setTimeout(() => { window.location.href = "diseno_viga_simplemente_armada.php"; }, 2000);</script>';
    } else {
        echo '<div class="alert alert-danger" role="alert">Error al actualizar: ' . $conexion->error . '</div>';
    }
}
?>
...
<div id="content" class="app-content">
	...
	<h1 class="page-header">
		Editar Diseño de Viga Simplemente Armada
	</h1>
					...
					<form id="form-diseno-viga" method="post" action="">
						<div class="row mb-3">
							<div class="col-md-12">
								...
								<select class="form-control" id="codigo_diseno" name="codigo_diseno" required>
									<option value="NTC CDMX 2017" <?= $viga['codigo_diseno'] === 'NTC CDMX 2017' ? 'selected' : '' ?>>NTC CDMX 2017</option>
									<option value="NTC CDMX 2023" <?= $viga['codigo_diseno'] === 'NTC CDMX 2023' ? 'selected' : '' ?>>NTC CDMX 2023</option>
								</select>
							</div>
						</div>
						...
						<div class="row mb-3">
							<div class="col-md-12">
								...
								<input type="text" class="form-control" id="obra" name="obra" value="<?= htmlspecialchars($viga['obra']) ?>">
							</div>
						</div>
						...
						<div class="row mb-3">
							<div class="col-md-6">
								...
								<input type="number" step="0.01" class="form-control" id="ancho_viga" name="ancho_viga" value="<?= htmlspecialchars($viga['ancho_viga']) ?>" required oninput="dibujarViga()">
							</div>
							<div class="col-md-6">
								<label for="altura_viga" class="form-label">Altura de la Viga (h) [cm] *</label>
								<input type="number" step="0.01" class="form-control" id="altura_viga" name="altura_viga" value="<?= htmlspecialchars($viga['altura_viga']) ?>" required oninput="dibujarViga()">
							</div>
						</div>
						...
						<label for="fc" class="form-label">Resistencia del Concreto (f'c) [kgf/cm²] *</label>
								<input type="number" step="0.01" class="form-control" id="fc" name="fc" value="<?= htmlspecialchars($viga['fc']) ?>" required>
							</div>
							<div class="col-md-6">
						<label for="fy" class="form-label">Resistencia del Acero (fy) [kgf/cm²] *</label>
								<input type="number" step="0.01" class="form-control" id="fy" name="fy" value="<?= htmlspecialchars($viga['fy']) ?>" required>
							</div>
						</div>
						...
						<div class="row mb-3">
							<div class="col-md-6">
								...
								<input type="number" step="0.01" class="form-control" id="recubrimiento_inferior" name="recubrimiento_inferior" value="<?= htmlspecialchars($viga['recubrimiento_inferior']) ?>" required oninput="dibujarViga()">
							</div>
							<div class="col-md-6">
						<label for="mu" class="form-label">Momento Último (Mu) [tonf*m] *</label>
								<input type="number" step="0.01" class="form-control" id="mu" name="mu" value="<?= htmlspecialchars($viga['mu']) ?>" required>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-md-12">
								<button type="submit" class="btn btn-primary">
									<i class="fas fa-save me-2"></i>Actualizar Diseño
								</button>
