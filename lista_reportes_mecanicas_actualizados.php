...
========================= */
$rutaFirebase = "Mecanicas/ReporteActualizadoMecanicas";
...
if ($usuarios) {
	foreach ($usuarios as $usuario => $reportes) {
		...
		foreach ($reportes as $llave => $reporte) {
			...
			$reporte['llave'] = $llave;
			$reportesActualizados[] = $reporte;
		}
	}
}

// Ordenar por fecha descendente (más reciente primero)
usort($reportesActualizados, function($a, $b) {
	$fechaA = $a['fecha'] ?? '';
	$fechaB = $b['fecha'] ?? '';
...
<style>
	...
	#datatableActualizadosMecanicas tbody td.dt-body-center {
		vertical-align: middle;
	}
</style>

<div id="content" class="app-content">
	<h1 class="page-header">Mecánica de suelos - Reportes actualizados</h1>
	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="table-responsive">
				<table id="datatableActualizadosMecanicas" class="table table-bordered table-striped table-hover nowrap w-100">
					...
					<tbody>
						<?php if ($reportesActualizados && count($reportesActualizados) > 0): ?>
							<?php foreach ($reportesActualizados as $r): ?>
								<?php 
								...
								<tr>
									...
									<td class="text-center">
										<a href="#"
											...
											onclick="
		window.open('validar_mecanica.php?usuario=<?= urlencode($r['usuario']) ?>&llave=<?= urlencode($r['llave']) ?>&tipo=Actualizado', '_blank');
		return false;
   ">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		const tableEl = document.querySelector('#datatableActualizadosMecanicas');
