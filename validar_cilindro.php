...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['item'])) {
	...
	try {
		...
		try {
			...
			// Consultar el precio de "Concreto (Cilindros)" en la tabla precios
			$sqlPrecio = "SELECT id FROM precios WHERE ensaye = 'Concreto (Cilindros)' AND expediente = ?";
			...
			// Verificar si ya existe un registro de "Concreto (Cilindros)" para el mismo expediente y fecha
			$sqlCheck = "SELECT id, cantidad FROM ensayes 
			             WHERE expediente = ? 
			             AND ensaye = 'Concreto (Cilindros)' 
			             AND fecha = ?";
			...
			if ($resultCheck->num_rows > 0) {
				...
				$sqlInsertEnsaye = "INSERT INTO ensayes 
				                    ...
				                    VALUES (?, ?, ?, ?, 'Concreto (Cilindros)', '1', '', ?, '', ?, NULL)";
